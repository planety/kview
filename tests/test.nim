...
proc loginNoExport(name: string): string {.writeExample.} =
  let vnode = buildHtml(tdiv(class = "mt-3")):
    h1: text "My Web Page"
    p: text "Hello " & name
    ul:
...
""".strip()
  ...
  test "can generate test_loginNoExport.html":
    discard loginNoExport(first)
    discard loginNoExport(second)
    check readFile("tests/test_loginNoExport.html").strip() == """<div class="mt-3">
  <h1>My Web Page</h1>
  <p>Hello first</p>
  <ul>
    <li>boston</li>
